...
exports.postSignup = function(req, res, next) {
  req.assert('name', 'Name is not valid').len(4);
  req.assert('email', 'Email is not valid').isEmail();
  ...
  User.findOne({ email: req.body.email }, function(err, existingUser) {
    ...
    User.findOne({"profile.name": user.profile.name}, function(err, result) {
      if (result) {
...
exports.postUpdateProfile = function(req, res, next) {
  User.findById(req.user.id, function(err, user) {
    ...
    User.findOne({"profile.name": user.profile.name}, function(err, result) {
      if (result && req.user.profile.name !== user.profile.name) {
        req.flash('errors', {msg: 'Username already exists'});
        return res.redirect('/account');
      }
      User.findOne({email: user.email}, function(err, check) {
        if (user && req.user.email !== check.email) {
          req.flash('errors', {msg: 'Account with that email address already exists.'});
          return res.redirect('/account');
        }
        user.save(function(err) {
