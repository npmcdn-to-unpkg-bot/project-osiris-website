...
emailForm.on('submit', function() {
  var pinCtrl = fs.readFileSync('controllers/pin.js').toString().split(os.EOL);
  ...
  var index = pinCtrl.indexOf('var transporter = nodemailer.createTransport({');
  pinCtrl.splice(index + 1, 1, "  service: '" + choice + "',");
  pinCtrl.splice(index + 3, 1, '    user: process.env.' + choice.toUpperCase() +'_USER,');
  pinCtrl.splice(index + 4, 1, '    pass: process.env.' + choice.toUpperCase() + '_PASSWORD');
  fs.writeFileSync('controllers/pin.js', pinCtrl.join(os.EOL));
